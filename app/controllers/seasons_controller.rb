class SeasonsController < ApplicationController
  ...
  def index
    @seasons = Season.all
    @seasons = @seasons.order(params[:by] + " " + params[:order]) if params[:sort]== "true"

    
    #@seasons = @seasons.where(:program_id => params[:program_id]) if params[:program_id]
  end
  ...
  private
    ...
    def season_params
      params.require(:season).permit(:number, :begin_at, :program_id)
    end
end
